...
class PairingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pairedAmbulances = DriverAmbulancePairing::where('pairing_date', $selectedDate)
            ...
            ->unique()
            ->toArray();
        // Drivers already paired with an ambulance (block these from being paired with another ambulance)
        ...
        if ($request->filled('get_options')) {
            ...
            $pairedMedicsForDate = DriverMedicPairing::where('pairing_date', $requestedDate)
                ->where('status', 'active')
                ->pluck('medic_id')
                ->unique()
                ->toArray();
                    ...
                    'options' => [
                        'medics' => $medics->map(function($medic) use ($pairedMedicsForDate) {
                            return [
                                ...
                                'isPaired' => in_array($medic->id, $pairedMedicsForDate),
                            ];
                        })->values(),
                    ]
                ]);
        ...
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'driverMedicPairings' => $groupedDriverMedicPairings->map(function($pairings) {
                    $firstPairing = $pairings->first();
                    return [
                        'groupKey' => $firstPairing->driver_id . '_' . $firstPairing->pairing_date->format('Y-m-d'),
                        'driver' => $firstPairing->driver ? [
                            'id' => $firstPairing->driver->id,
                            'name' => $firstPairing->driver->name,
                            'phone' => $firstPairing->driver->phone,
                        ] : null,
                        'medics' => $pairings->pluck('medic')->filter()->map(function($medic) {
                            return [
                                'id' => $medic->id,
                                'name' => $medic->name,
                                'specialization' => $medic->specialization,
                        ...
                        'pairing_date' => $firstPairing->pairing_date->format('Y-m-d'),
                        'pairing_date_formatted' => $firstPairing->pairing_date->format('M d, Y'),
                        'pairing_date_day' => $firstPairing->pairing_date->format('l'),
                        'start_time' => $firstPairing->start_time,
                        'end_time' => $firstPairing->end_time,
                        'status' => $pairings->pluck('status')->unique()->values()->toArray(),
                        'isActive' => $pairings->pluck('status')->contains('active'),
                    ];
                })->values(),
                'driverAmbulancePairings' => $groupedDriverAmbulancePairings->map(function($pairings) use ($groupOperators) {
                    $firstPairing = $pairings->first();
                    $groupKey = $firstPairing->ambulance_id . '_' . $firstPairing->pairing_date->format('Y-m-d');
                    $groupDrivers = isset($groupOperators[$groupKey]) ? $groupOperators[$groupKey] : $pairings->pluck('driver');
                    return [
                        'groupKey' => $groupKey,
                        'drivers' => $groupDrivers->filter()->map(function($driver) {
                            return [
                                'id' => $driver->id,
                                'name' => $driver->name,
                                'phone' => $driver->phone,
                        ...
                        'ambulance' => $firstPairing->ambulance ? [
                            'id' => $firstPairing->ambulance->id,
                            'name' => $firstPairing->ambulance->name,
                            'plate_number' => $firstPairing->ambulance->plate_number,
                        ] : null,
                        'pairing_date' => $firstPairing->pairing_date->format('Y-m-d'),
                        'pairing_date_formatted' => $firstPairing->pairing_date->format('M d, Y'),
                        'pairing_date_day' => $firstPairing->pairing_date->format('l'),
    ...
    public function createDriverMedicPairing(Request $request)
    {
        ...
        // Get already paired medics for the selected date (medics can only be paired with one driver)
        ...
        // Note: Drivers can have multiple medics, so we don't block drivers
        // Drivers paired with ambulances can still be paired with medics
        
        return view('admin.pairing.create-driver-medic', compact('drivers', 'medics', 'pairedMedics', 'selectedDate'));
    }
    ...
    public function storeDriverMedicPairing(Request $request)
    {
        ...
        $existingExactPairing = DriverMedicPairing::where('driver_id', $request->driver_id)
            ->where('medic_id', $request->medic_id)
            ->where('pairing_date', $request->pairing_date)
            ->where('status', 'active')
            ->first();

        if ($existingExactPairing) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'This driver-medic pairing already exists on this date.',
                    'errors' => ['medic_id' => ['This driver-medic pairing already exists on this date.']]
                ], 422);
            }
            return redirect()->back()
                ->withInput()
                ->withErrors(['medic_id' => 'This driver-medic pairing already exists on this date.']);
        }

        // Check if medic is already paired with a different driver (medics can only be paired with one driver)
        ...
        if ($existingMedicPairing) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'This medic is already paired with another driver on this date.',
                    'errors' => ['medic_id' => ['This medic is already paired with another driver on this date.']]
                ], 422);
            }
            return redirect()->back()
                ->withInput()
        ...
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Driver-Medic pairing created successfully!'
            ]);
        }
    ...
    public function createDriverAmbulancePairing(Request $request)
    {
        ...
        // Get drivers already paired with another ambulance (block these)
        ...
        // Get ambulances that already have 2 drivers (block these)
        $ambulancesWithTwoDrivers = DriverAmbulancePairing::where('pairing_date', $selectedDate)
            ->where('status', 'active')
            ->selectRaw('ambulance_id, COUNT(*) as driver_count')
            ->groupBy('ambulance_id')
        ...
        // Get all paired ambulances for display purposes
        $pairedAmbulances = DriverAmbulancePairing::where('pairing_date', $selectedDate)
            ->where('status', 'active')
            ->pluck('ambulance_id')
            ->unique()
        ...
        return view('admin.pairing.create-driver-ambulance', compact('drivers', 'ambulances', 'pairedDrivers', 'pairedAmbulances', 'selectedDate', 'ambulancesWithTwoDrivers'));
    }
    ...
    public function storeDriverAmbulancePairing(Request $request)
    {
        ...
        // Check if this exact pairing already exists
        $existingExactPairing = DriverAmbulancePairing::where('driver_id', $request->driver_id)
        ...
        if ($existingExactPairing) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'This driver-ambulance pairing already exists on this date.',
                    'errors' => ['driver_id' => ['This driver-ambulance pairing already exists on this date.']]
                ], 422);
            }
            return redirect()->back()
                ->withInput()
        ...
        if ($existingDriverPairing) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'This driver is already paired with another ambulance on this date.',
                    'errors' => ['driver_id' => ['This driver is already paired with another ambulance on this date.']]
                ], 422);
            }
            return redirect()->back()
                ->withInput()
        ...
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'success' => true,
                'message' => 'Driver-Ambulance pairing created successfully!'
            ]);
        }
            ...
            foreach ($request->drivers as $driverId) {
                foreach ($request->ambulances as $ambulanceId) {
                    try {
                        ...
                        // Check if driver is already paired with a different ambulance
                        $existingDriverPairing = DriverAmbulancePairing::where('driver_id', $driverId)
                            ->where('pairing_date', $request->pairing_date)
                            ->where('status', 'active')
                            ->where('ambulance_id', '!=', $ambulanceId)
                            ->first();
                        ...
                        if ($existingDriverPairing) {
                            $errors[] = "Driver {$driverId} is already paired with another ambulance on this date";
                            continue;
                        }
