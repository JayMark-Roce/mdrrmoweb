...
#map-container .leaflet-popup.mdrrmo-base-popup .leaflet-popup-content-wrapper {
  ...
  box-shadow: 0 20px 60px rgba(0, 0, 0, 0.15), 0 8px 24px rgba(0, 0, 0, 0.1), 0 2px 8px rgba(0, 0, 0, 0.05) !important;
  padding: 0 !important;
...
/* Arrow tip pointing down to marker - beautiful design */
...
.modal-content {
    ...
    border-radius: 24px;
    box-shadow: 0 32px 64px rgba(0, 0, 0, 0.25), 0 16px 32px rgba(0, 0, 0, 0.15), 0 4px 16px rgba(0, 0, 0, 0.1);
    max-height: 85vh;
    width: 100%;
    max-width: 600px;
    overflow-y: auto;
...
@keyframes inlineModalSlide {
    ...
    .compact-card::before {
        ...
        width: 3px;
        height: 100%;
        background: linear-gradient(180deg, var(--brand-orange, #f97316), #ea580c);
        opacity: 0;
    ...
    .gps-map-shell {
        border-radius: 24px;
        border: 1px solid rgba(226, 232, 240, 0.6);
        box-shadow: 0 20px 40px rgba(15, 23, 42, 0.1), 0 8px 16px rgba(0, 0, 0, 0.06);
        overflow: hidden;
        background: #ffffff;
        position: relative;
        flex: 1;
    ...
    .gps-toolbar-actions button.secondary {
        background: linear-gradient(135deg, #f8fafc, #f1f5f9);
        color: #0f172a;
        border: 1px solid rgba(226, 232, 240, 0.8);
    ...
    .gps-toolbar-actions button:hover {
        transform: translateY(-2px);
        box-shadow: 0 8px 20px rgba(59, 130, 246, 0.4), 0 4px 8px rgba(59, 130, 246, 0.3);
    }

    .gps-toolbar-actions button.secondary:hover {
        background: linear-gradient(135deg, #ffffff, #f8fafc);
        box-shadow: 0 4px 12px rgba(15, 23, 42, 0.12);
        border-color: rgba(59, 130, 246, 0.3);
    }

    .gps-driver-actions-panel {
        margin-top: 1rem;
        border-radius: 20px;
    ...
    .gps-driver-actions-panel::-webkit-scrollbar {
        width: 6px;
    }

    .gps-driver-actions-panel::-webkit-scrollbar-thumb {
        background: rgba(148, 163, 184, 0.6);
        border-radius: 999px;
    }

    /* Case modal enhancements */
    .case-modal-shell {
        background: #f8fafc;
        border-radius: 24px;
        border: 1px solid rgba(148, 163, 184, 0.35);
    ...
    .case-pin-action--move {
        background: linear-gradient(135deg, #f59e0b, #fb923c);
        color: #ffffff;
    }

    .case-pin-action--cancel {
        background: linear-gradient(135deg, #ef4444, #b91c1c);
        color: #ffffff;
    }

    .case-pin-actions button:hover {
        transform: translateY(-2px);
    }
    ...
    .case-details-section--patient {
        border-left: 4px solid #2563eb;
    }

    .case-details-section--location {
        border-left: 4px solid #ef4444;
        background: #fff7ed;
    }

    .case-details-section--assignment {
        border-left: 4px solid #10b981;
        background: #ecfdf5;
    }

    .case-details-section--timeline {
    ...
    .case-details-list div {
        display: grid;
        gap: 0.1rem;
    }

    .case-details-list dt {
        font-size: 0.7rem;
        letter-spacing: 0.08em;
        color: #94a3b8;
        text-transform: uppercase;
    ...
    .case-details-id-block {
        display: flex;
        flex-direction: column;
        gap: 0.15rem;
    }

    .case-details-id-block span {
        font-size: 0.7rem;
    ...
    .case-details-id-block strong {
        display: block;
        font-size: 1.35rem;
        color: #0f172a;
    }

    .case-details-id-block small {
        color: #94a3b8;
        font-weight: 600;
    }
    ...
    .gps-dashboard-panel.collapsed .gm-status-bar {
        display: none;
    }
    ...
    .gm-status-item:nth-child(2) .gm-status-icon {
        background: rgba(248, 113, 113, 0.15);
        color: #ef4444;
        box-shadow: inset 0 0 0 1px rgba(239, 68, 68, 0.2);
    }

    .gm-status-item:nth-child(3) .gm-status-icon {
    ...
    /* Neo map styling */
    .gps-map-shell .leaflet-popup-content-wrapper {
    ...
    .gps-map-shell .leaflet-popup-content {
        margin: 0;
        padding: 1rem 1.2rem;
        color: #0f172a;
        font-weight: 600;
    ...
    .gps-map-shell .leaflet-popup-tip {
        background: #ffffff;
        border: 1px solid rgba(99, 102, 241, 0.2);
    }
    ...
    .neo-driver-icon.is-stale {
        filter: grayscale(0.8);
        opacity: 0.75;
        background: linear-gradient(135deg, #64748b, #475569);
    }

    .neo-driver-icon::after {
        content: '';
        position: absolute;
        inset: -2px;
    ...
    .neo-driver-icon .neo-driver-pulse {
        position: absolute;
        width: 90%;
        height: 90%;
        border-radius: 18px;
    ...
    .neo-drop-pin.destination { --pin-color: linear-gradient(135deg, #d946ef, #9333ea); }
    .neo-drop-pin.waypoint { --pin-color: linear-gradient(135deg, #10b981, #059669); }

    .neo-drop-pin.centered-pin {
        gap: 0;
    }

    .neo-drop-pin.centered-pin::after {
        display: none;
    }
    ...
    .map-modal {
        ...
        -webkit-backdrop-filter: blur(20px) saturate(180%);
        border: 1px solid rgba(226, 232, 240, 0.6);
        box-shadow: 0 32px 64px rgba(15, 23, 42, 0.2), 0 16px 32px rgba(0, 0, 0, 0.12), 0 4px 16px rgba(0, 0, 0, 0.08);
        display: flex;
        flex-direction: column;
        overflow: hidden;
        animation: slideUp 0.4s cubic-bezier(0.16, 1, 0.3, 1);
    ...
    #update-indicator {
        ...
        border-left: 4px solid var(--gps-green);
...
<!-- Main content -->
<main class="maincontentt">
    <div class="gps-dashboard">
        <section class="gps-map-card">
            <!-- Compact Status Cards -->
            <div class="gm-status-bar compact-status-cards" id="map-dashboard-body">
                <div class="compact-card">
                    <div class="compact-icon" style="background: linear-gradient(135deg, #6366f1, #4f46e5);">
                        <i class="fas fa-map-marker-alt"></i>
                    </div>
                    <div class="compact-info">
                        <span class="compact-value" id="totalCases">0</span>
                        <span class="compact-label">Total Cases</span>
                    </div>
                </div>
                <div class="compact-card">
                    <div class="compact-icon" style="background: linear-gradient(135deg, #f59e0b, #d97706);">
                        <i class="fas fa-user"></i>
                    </div>
                    <div class="compact-info">
                        <span class="compact-value" id="totalDrivers">0</span>
                        <span class="compact-label">Active Drivers</span>
                    </div>
                </div>
                <div class="compact-card">
                    <div class="compact-icon" style="background: linear-gradient(135deg, #ef4444, #dc2626);">
                        <i class="fas fa-exclamation-circle"></i>
                    </div>
                    <div class="compact-info">
                        <span class="compact-value" id="activeCases">0</span>
                        <span class="compact-label">Active Cases</span>
                    </div>
                </div>
            </div>

            <!-- Desktop Toolbar (Hidden on Mobile) -->
            <div class="gps-map-toolbar desktop-only">
                <div>
                    <h3 style="margin:0;">Live Response Map</h3>
                </div>
                <div class="gps-toolbar-actions">
                    <button type="button" onclick="openModal('case-creation-modal')">
                        <i class="fas fa-plus-circle"></i>
                        Create Case
                    </button>
                    <button type="button" onclick="openModal('active-modal')">
                        <i class="fas fa-list-check"></i>
                        Active Cases
                    </button>
                    <button type="button" class="secondary" onclick="openModal('geocode-modal')">
                        <i class="fas fa-map-pin"></i>
                        Pin by Address
                    </button>
                    <button type="button" class="secondary" id="map-dashboard-toggle" aria-expanded="true" title="Hide dashboard metrics">
                        <i class="fas fa-eye-slash" id="map-dashboard-toggle-icon"></i>
                        <span id="map-dashboard-toggle-label">Hide Dashboard</span>
                    </button>
                    <button type="button" class="secondary" onclick="toggleFullscreen(event)">
            ...
            <div id="map-container" class="rounded shadow modern-map-container" data-default-style="min-height: calc(100vh - 220px); width: 100%; max-width: 100%; margin: 0; border-radius: 20px; border: 1px solid rgba(148, 163, 184, 0.2); box-shadow: 0 20px 40px rgba(15, 23, 42, 0.12), 0 8px 16px rgba(0, 0, 0, 0.08); background: #ffffff; display: block; position: relative; overflow: hidden;" style="min-height: calc(100vh - 220px); width: 100%; max-width: 100%; margin: 0; border-radius: 20px; border: 1px solid rgba(148, 163, 184, 0.2); box-shadow: 0 20px 40px rgba(15, 23, 42, 0.12), 0 8px 16px rgba(0, 0, 0, 0.08); background: #ffffff; display: block; position: relative; overflow: hidden;">
                
                <!-- Mobile Floating Action Buttons -->
                <div class="mobile-fab-container">
                    <div class="fab-main" id="mobile-fab-main" onclick="toggleMobileFAB()">
                        <i class="fas fa-plus"></i>
                    </div>
                    <div class="fab-menu" id="mobile-fab-menu">
                        <button class="fab-item" onclick="openModal('case-creation-modal'); toggleMobileFAB();" title="Create Case">
                            <i class="fas fa-plus-circle"></i>
                            <span>Create Case</span>
                        </button>
                        <button class="fab-item" onclick="openModal('active-modal'); toggleMobileFAB();" title="Active Cases">
                            <i class="fas fa-list-check"></i>
                            <span>Active Cases</span>
                        </button>
                        <button class="fab-item" onclick="openModal('geocode-modal'); toggleMobileFAB();" title="Pin by Address">
                            <i class="fas fa-map-pin"></i>
                            <span>Pin Address</span>
                        </button>
                        <button class="fab-item" onclick="toggleFullscreen(event); toggleMobileFAB();" title="Fullscreen">
                            <i class="fas fa-expand"></i>
                ...
                <div id="caseCreationContainer" class="case-modal-shell" title="Create New Case">
                    ...
                    <div class="case-modal-body">
                        ...
                        <form id="case-creation-form" class="case-form-grid">
                            ...
                            <div class="form-actions case-form-actions">
...
const defaultMapStyle = mapContainerEl ? (mapContainerEl.dataset.defaultStyle || mapContainerEl.getAttribute('style') || '') : '';
...
// GPS Panel removed - functions moved to toolbar
...
function toggleFullscreen(event) {
    ...
    const mapContainer = mapContainerEl || document.getElementById('map-container');
    ...
    if (body.classList.contains('fullscreen-mode')) {
        ...
        mapContainer.style.cssText = defaultMapStyle;
...
function positionTopControls(){
    try {
        const statusBar = document.querySelector('#map-container .gm-status-bar');
        const hasOverlayBar = Boolean(statusBar);
        const rect = hasOverlayBar && statusBar.getBoundingClientRect ? statusBar.getBoundingClientRect() : null;
        const barH = rect && rect.height ? rect.height : (hasOverlayBar ? statusBar.offsetHeight : 0);
        const pad = 24; // spacing below bar
        const topPx = (Math.round(hasOverlayBar ? barH : 0) + pad) + 'px';
...
const dashboardPanelEl = document.getElementById('map-dashboard-body');
const dashboardToggleBtn = document.getElementById('map-dashboard-toggle');
const dashboardToggleIcon = document.getElementById('map-dashboard-toggle-icon');
const dashboardToggleLabel = document.getElementById('map-dashboard-toggle-label');

if (dashboardPanelEl && dashboardToggleBtn) {
    dashboardToggleBtn.addEventListener('click', function(event) {
        event.preventDefault();
        event.stopPropagation();
        const collapsed = dashboardPanelEl.classList.toggle('collapsed');
        dashboardToggleBtn.setAttribute('aria-expanded', (!collapsed).toString());

        if (dashboardToggleIcon) {
            dashboardToggleIcon.classList.toggle('fa-eye-slash', !collapsed);
            dashboardToggleIcon.classList.toggle('fa-eye', collapsed);
        }

        if (dashboardToggleLabel) {
            dashboardToggleLabel.textContent = collapsed ? 'Show Dashboard' : 'Hide Dashboard';
        }

        dashboardToggleBtn.title = collapsed ? 'Show dashboard metrics' : 'Hide dashboard metrics';
    });
}
...
function openCaseDetailsModal(caseData) {
    ...
    const createdAt = caseData.created_at ? new Date(caseData.created_at) : null;
    const updatedAt = caseData.updated_at ? new Date(caseData.updated_at) : null;
    const pickupCoords = (caseData.latitude && caseData.longitude)
        ? `${parseFloat(caseData.latitude).toFixed(4)}, ${parseFloat(caseData.longitude).toFixed(4)}`
        : 'Not set';
    const destinationCoords = (caseData.to_go_to_latitude && caseData.to_go_to_longitude)
        ? `${parseFloat(caseData.to_go_to_latitude).toFixed(4)}, ${parseFloat(caseData.to_go_to_longitude).toFixed(4)}`
        : 'Not set';
    const destinationAddress = caseData.destination || caseData.to_go_to || caseData.destination_address || 'Not specified';
    const ambulanceName = caseData.ambulance ? caseData.ambulance.name : 'Unassigned';
    ...
    const caseDetails = `
        <div class="case-details-card compact">
            <div class="case-details-meta">
                <div class="case-details-id-block">
                    <span>Case Number</span>
                    <strong>#${caseData.case_num}</strong>
                    <small>${updatedAt ? `Updated ${updatedAt.toLocaleString()}` : 'Awaiting update'}</small>
                </div>
                <div class="case-details-tags">
                    <span class="case-tag case-tag--priority" style="--tag-color:${getPriorityColor(priority)}">${priority}</span>
                    ${caseData.type ? `<span class="case-tag"><i class="fas fa-notes-medical"></i> ${caseData.type}</span>` : ''}
                    <span class="case-tag" style="background: rgba(59,130,246,0.12); color:#1d4ed8;">
            ...
            <div class="case-stat-grid">
                ...
                <div class="case-stat-tile">
                    <label>Pickup Coords</label>
                    <span>${pickupCoords}</span>
                </div>
                <div class="case-stat-tile">
                    <label>Destination</label>
                    <span>${destinationCoords}</span>
                </div>
            </div>

            <div class="case-panels">
                <section class="case-panel">
                    <h5><i class="fas fa-user"></i> Caller & Patient</h5>
                    <dl>
                        <div>
                            <dt>Caller</dt>
                            <dd>${caseData.caller_name || 'Not specified'}</dd>
                        </div>
                        <div>
                            <dt>Caller Contact</dt>
                            <dd>${caseData.caller_contact || 'Not provided'}</dd>
                        </div>
                        <div>
                            <dt>Patient Name</dt>
                            <dd>${caseData.name || 'Not specified'}</dd>
                        </div>
                        <div>
                            <dt>Patient Contact</dt>
                            <dd>${caseData.contact || 'Not provided'}</dd>
                        </div>
                        <div>
                            <dt>Pickup Address</dt>
                            <dd>${caseData.address || 'No pickup address supplied'}</dd>
                        </div>
                    </dl>
                </section>

                <section class="case-panel">
                    <h5><i class="fas fa-location-dot"></i> Locations</h5>
                    <dl>
                        <div>
                            <dt>Pickup</dt>
                            <dd>${pickupCoords}</dd>
                ...
                <section class="case-panel">
                    <h5><i class="fas fa-truck-medical"></i> Assignment</h5>
                    ${caseData.ambulance ? `
                    <dl>
                        <div>
                            <dt>Ambulance</dt>
                    ...
                    ` : `<p style="margin:0; font-weight:700; color:#0f172a;">No ambulance assigned yet.</p>`}
                </section>
                ...
                <section class="case-panel">
                    ...
                    <dl>
                        <div>
                            <dt>Created</dt>
                            <dd>${createdAt ? createdAt.toLocaleString() : 'N/A'}</dd>
                        </div>
                        <div>
                            <dt>Updated</dt>
                            <dd>${updatedAt ? updatedAt.toLocaleString() : 'N/A'}</dd>
                        </div>
                    </dl>
                </section>
            </div>
            ...
            <div class="case-details-actions">
                ...
                <button onclick="updateCaseStatus(${caseData.case_num})" style="background: linear-gradient(135deg, #10b981, #059669); color:#fff;">
                    <i class="fas fa-edit"></i> Update
                </button>
                <button onclick="deleteCase(${caseData.case_num})" style="background: linear-gradient(135deg, #ef4444, #dc2626); color:#fff;">
                    <i class="fas fa-trash"></i> Delete
                </button>
                <button id="case-details-print-btn" style="background: linear-gradient(135deg, #2563eb, #1d4ed8); color:#fff;">
    ...
    const detailsPrintBtn = document.getElementById('case-details-print-btn');
    if (detailsPrintBtn) {
        detailsPrintBtn.addEventListener('click', (event) => {
            event.preventDefault();
...
document.getElementById('move-pin-btn')?.addEventListener('click', function(event) {
    ...
    if (isMovingPin) {
        ...
        document.body.classList.add('pinning-mode');
        // Enter moving mode
        isMovingPin = true;
...
const MAPTILER_KEY = 'rXuxRMm146udu19x36Fj';
...
// Auto-switch to fallback if MapTiler fails (e.g., API limit)
...
function autoFitMapBounds() {
    if (!autoFitBoundsEnabled || isUserInteracting || document.body.classList.contains('pinning-mode')) return;
...
function updateStatusCounts(){
    ...
    const driverCount = Object.keys(driverMarkers || {}).length;

    if (totalCasesEl) totalCasesEl.textContent = String(totalCasesCount);
    if (totalDriversEl) totalDriversEl.textContent = String(driverCount);
...
// Helper to create modern drop pins
function createDropPin(variant, iconClass, options = {}) {
    const classes = ['neo-drop-pin'];
    ...
    return L.divIcon({
        className: '',
        html: `<div class="${classes.join(' ')}"><i class="fas ${iconClass}"></i></div>`,
        iconSize,
        iconAnchor,
        popupAnchor
    });
...
const pinIcon = createDropPin('pickup', 'fa-map-pin');
const destinationIcon = createDropPin('destination', 'fa-flag-checkered', { centered: true });
const destIcon = createDropPin('waypoint', 'fa-location-arrow');

// Case marker icon - premium style
...
function getDriverInitials(label = '') {
...
function createDriverIcon(label, photoUrl, ageSec = 0, ambId = null) {
    ...
    if (ambId) {
        ...
        const badgeColor = colorMap[health] || colorMap.unknown;
        const labelMap = {
            good: 'Good connection',
            warning: 'Intermittent connection',
            poor: 'Poor connection',
            unknown: 'Connection status unknown'
        };
        healthBadge = `<span style="position:absolute; top:-6px; right:-6px; width:14px; height:14px; border-radius:50%; background:${badgeColor}; border:2px solid #ffffff; box-shadow:0 4px 10px rgba(0,0,0,0.35);" title="${labelMap[health] || labelMap.unknown}"></span>`;
    }
    
    const html = `
      <div class="driver-marker" style="display:flex; flex-direction:column; align-items:center; gap:6px; position:relative; cursor: pointer;">
        <div style="position:relative;">
...
map.on('click', async function (e) {
    ...
    // Panel removed - map clicks are always processed
...
document.getElementById('case-creation-form')?.addEventListener('submit', async function(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const patientName = formData.name || 'Unidentified Patient';
            const patientContact = formData.contact || 'N/A';
            const popupCallerName = formData.caller_name;
            const popupCallerContact = formData.caller_contact;

            const popupContent = `
                <div style="min-width: 200px;">
                    <h4 style="margin: 0 0 0.5rem 0; color: #1e293b;">Case #${result.case_num}</h4>
                    <p style="margin: 0 0 0.3rem 0; font-weight: 600;">Caller: ${popupCallerName}</p>
                    <p style="margin: 0 0 0.3rem 0; color: #6b7280; font-size: 0.9rem;">Contact: ${popupCallerContact}</p>
                    <p style="margin: 0 0 0.3rem 0; font-weight: 600;">Patient: ${patientName}</p>
                    ${formData.contact ? `<p style="margin: 0 0 0.3rem 0; color: #6b7280; font-size: 0.9rem;">Patient Contact: ${patientContact}</p>` : ''}
...
async function printCaseDetails(caseNum) {
    try {
        ...
        showInlineNotice('Failed to load case details for printing. Please try again.', {
            title: 'Print Error',
            type: 'danger'
        });
    }
...
// Generate printable HTML for case details (formal report)
function generateCaseDetailsPrintHtml(caseData) {
    ...
    // Get pairing information - fetch from API if not available
    let driverName = 'Not assigned';
    let driverPhone = 'N/A';
    let medics = [];
    
    // Try to get pairing data from caseData first
    if (caseData.driver_pairing) {
        driverName = caseData.driver_pairing.name || 'Not assigned';
        driverPhone = caseData.driver_pairing.phone || 'N/A';
    }
    
    if (caseData.medic_pairings && Array.isArray(caseData.medic_pairings) && caseData.medic_pairings.length > 0) {
        medics = caseData.medic_pairings;
    } else if (caseData.medic_pairing) {
        medics = [caseData.medic_pairing];
    }
                ...
                <div class="sections-grid">
                    <div class="info-section">
                        ...
                        <div class="info-row">
                            ...
                            <span class="info-value"><strong style="color: #10b981;">${caseData.status === 'Completed' ? '✓ Completed' : caseData.status || 'Pending'}</strong></span>
                        </div>
                        <div class="info-row">
...
function printCaseFromOptions(format = 'conduction') {
...
document.getElementById('close-print-modal')?.addEventListener('click', (event) => {
    event.preventDefault();
    closePrintOptionsModal();
});

document.getElementById('print-conduction-option')?.addEventListener('click', (event) => {
    event.preventDefault();
    printCaseFromOptions('conduction');
});

document.getElementById('print-details-option')?.addEventListener('click', (event) => {
    event.preventDefault();
    printCaseFromOptions('details');
});

document.getElementById('print-options-modal')?.addEventListener('click', function(event) {
