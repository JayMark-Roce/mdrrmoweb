            ...
            <div>
              ...
              <select name="driver_id" id="driver_id" required class="w-full">
                ...
                @foreach($drivers as $driver)
                  @php
                    // Block drivers who are already paired with another ambulance on this date
                    $isBlocked = in_array($driver->id, $pairedDrivers);
                  @endphp
                  @if($isBlocked)
                    <option value="{{ $driver->id }}" disabled class="text-gray-400 bg-gray-100">{{ $driver->name }} (Already paired with another ambulance on {{ $selectedDate }})</option>
                  @else
              ...
              <p class="text-xs text-gray-500 mt-1">Note: Drivers paired with medics can still be paired with ambulances</p>
            </div>
            <div>
              ...
              <select name="ambulance_id" id="ambulance_id" required class="w-full">
                ...
                @foreach($ambulances as $ambulance)
                  @php
                    // Block ambulances that already have 2 drivers (max limit)
                    $isBlocked = isset($ambulancesWithTwoDrivers) && in_array($ambulance->id, $ambulancesWithTwoDrivers);
                    $driverCount = isset($pairedAmbulances) && in_array($ambulance->id, $pairedAmbulances) ? 
                      \App\Models\DriverAmbulancePairing::where('ambulance_id', $ambulance->id)
                        ->where('pairing_date', $selectedDate)
                        ->where('status', 'active')
                        ->count() : 0;
                  @endphp
                  @if($isBlocked)
                    <option value="{{ $ambulance->id }}" disabled class="text-gray-400 bg-gray-100">{{ $ambulance->name }} - Already has 2 drivers (max limit) on {{ $selectedDate }}</option>
                  @else
                    <option value="{{ $ambulance->id }}" {{ old('ambulance_id') == $ambulance->id ? 'selected' : '' }}>
                      {{ $ambulance->name }}@if($driverCount > 0) ({{ $driverCount }}/2 drivers)@endif
                    </option>
                  @endif
                @endforeach
              </select>
              @error('ambulance_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
              <p class="text-xs text-gray-500 mt-1">Maximum 2 drivers per ambulance</p>
            </div>
          </div>
